...
def derive_light_state(data: dict[str, Any], now: float | None = None) -> dict[str, Any]:
    ...
    bomb_countdown = to_float(deep_get(data, "bomb", "countdown", default=-1), default=-1)
    ...
    if planted:
        light_mode = "blink"
        light_color = "red"
        time_left = bomb_countdown if bomb_countdown >= 0 else infer_time_left(now, _bomb_planted_monotonic)
        ...
        light_mode = "steady"
        light_color = "red"
    elif round_phase.lower() in ("over", "gameover") and win_team in ("CT", "T"):
        light_mode = "steady"
        light_color = "blue" if win_team == "CT" else "green"
    elif flashed > 0:
        light_mode = "flash"
